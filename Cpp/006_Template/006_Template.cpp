...
#include "TemplateFunctionObject.h"

// 함수 오버로딩을 이용하면 매개변수 리스트가 다른 3 함수를 정의할 수 있지만,
...
// 템플릿은 완벽한 도형의 틀을 가진 모양자의 틀에 그림을 찍어내듯이 
// 완성도 높은 코드를 사용자의 마음대로 특정 데이터 타입을 지정하여 
// 해당 코드를 재사용하는 제네릭 프로그래밍의 기초이다.

// 물론 템플릿을 사용하면 템플릿 코드를 특정 매개변수 데이터 타입으로 재정의하게 되서
// 컴파일 시간이 길어지지만, 코드부분에서의 용량은 미미하기 때문에 
// 템플릿을 사용하면 생산성이 높고 유지보수가 뛰어나기 때문에 필자는 적극적으로 사용하고 있다.


// 템플릿은 특정 데이터 타입을 지정하지 않고 사용자가 일반적인 데이터 타입으로 작업을 수행할 
// 구체적인 매개변수 타입을 지정하여 사용성 높이는 제네릭 프로그래밍의 기초이다.